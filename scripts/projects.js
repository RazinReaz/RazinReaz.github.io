const projects = [
  {
    imageSrc: "./assets/projects/pathtracer-gpu/pathtracer-vid.mp4",
  ...
  {
    imageSrc: "./assets/projects/black-hole/blackhole.mp4",
    imageAlt: "Black Hole",
    title: "Real-time Black Hole Simulation",
    tools: ["CUDA C++", "OpenGL"],
    description:
      "A ray marched black hole simulation with gavitational lensing with geodesic equation and volumetric rendering of accretion disk and ",
    links: [
      {
        href: "https://github.com/RazinReaz/black-hole",
        text: "Github",
      },
      {
        href: "https://www.youtube.com/shorts/HplpgIAp3jM",
        text: "Video",
      },
    ],
  },
  ...
  {
    imageSrc: "./assets/projects/flocking-boids/flocking-boids.mp4",
  ...
  {
    imageSrc: "./assets/projects/ray-casting/ray-casting.mp4",
...
function createProjectCard(project) {
  ...
  const isVideo = isVideoSource(project.imageSrc);
  const mediaElement = isVideo 
      ? `<video data-src="${project.imageSrc}" class="project-media project-image" muted loop playsinline autoplay></video>`
      : `<img data-src="${project.imageSrc}" alt="${project.imageAlt}" class="project-media project-image" loading="lazy">`;
...
// Lazy loading implementation for project images and videos
function setupProjectLazyLoading() {
    const mediaObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (media.dataset.src) {
                    ...
                    media.classList.remove('project-image');
                    media.classList.add('project-image-loaded');
        ...
        rootMargin: '100px 0px', // Start loading 100px before media comes into view
        threshold: 0.01
    });

    // Observe all project images and videos
    document.querySelectorAll('.project-image').forEach(media => {
        mediaObserver.observe(media);
    });
}

// Progressive loading - load first few project images/videos immediately
function loadCriticalProjectImages() {
    ...
    criticalProjects.forEach((project, index) => {
        setTimeout(() => {
            const media = document.querySelector(`[data-src="${project.imageSrc}"]`);
            if (media) {
                media.src = media.dataset.src;
                media.classList.remove('project-image');
                media.classList.add('project-image-loaded');
